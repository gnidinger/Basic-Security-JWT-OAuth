...
public class OAuth2Service implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        OAuth2UserProfile oAuthUserProfile = OAuth2Attributes.extract(registrationId, attributes); // registrationId에 따라 유저 정보를 통해 공통된 UserProfile 객체로 만들어 줌

        User user = saveOrUpdate(oAuthUserProfile); // DB에 저장

        return new DefaultOAuth2User(
                Collections.singleton(new SimpleGrantedAuthority(user.getAuthType().toString())),
                attributes, userNameAttributeName);
    }

    /*oauth 이메일(아이디)로 회원가입 전 중복체크하고 oauth 계정에서 닉네임 등 변동 있을시 업데이트*/
    private User saveOrUpdate(OAuth2UserProfile userProfile) {
        User user = userRepository.findByUserId(userProfile.getEmail())
                .map(m -> m.oauthUpdate(userProfile.getName(), userProfile.getEmail())) // OAuth 서비스 사이트에서 유저 정보 변경이 있을 수 있기 때문에 우리 DB에도 update
                .orElse(userProfile.createOauth2User());
